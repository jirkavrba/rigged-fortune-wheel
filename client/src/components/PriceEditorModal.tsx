    ...
    return (
        <div className="flex flex-col items-center group my-5">
            ...
            <ul className="flex flex-row items-center justify-between p-5 rounded-full bg-gray-50">
                {colors.map((color, i) =>
                    <li key={i}
                        className={`w-8 h-8 mx-2 rounded-full transform transition cursor-pointer ${selected === color ? 'opacity-100 scale-125' : 'group-hover:opacity-100 opacity-50 hover:scale-90'}`}
        ...
        <div
            ...
            <div
                ...
                <input value={title} onChange={(event) => setTitle(event.target.value)}
                       className="border outline-none focus:ring-4 focus:ring-gray-300 rounded-xl text-3xl font-black px-8 py-4 mb-10 w-full"/>

                <h1 className="uppercase text-gray-500 font-bold text-xs tracking-widest mb-3">Popis</h1>
                <textarea style={{resize: "none"}} rows={5} value={description}
                          onChange={(event) => setDescription(event.target.value)}
                          className="border outline-none focus:ring-4 focus:ring-gray-300 rounded-xl text-lg text-gray-500 font-bold px-8 py-4 mb-10 w-full"/>

                <div className="flex flex-row justify-center items-center">
                    <button onClick={submit}
                            className="px-10 py-4 bg-black text-white uppercase font-black tracking-wide rounded-xl shadow-xl transition transform hover:scale-95">Přidat
                    </button>
